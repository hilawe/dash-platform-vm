...
| D-27 | Direction Decision 0 is a direction decision, not a gate, and its method is committed before its corpus | ACCEPTED (method, 2026-08-30) | Criteria written after evidence describe the evidence. The method is `docs/DIRECTION_DECISION_0_METHOD.md` and contains no conclusion | None. This one exists to keep the ordering honest |
| D-28 | The direction question has two axes, capability and admission, evaluated separately | ACCEPTED (method, 2026-08-30) | A single four-option list conflated them, which cannot express a restricted general VM or a permissionless native catalog | None foreseen |
| D-29 | P8 is REOPENED by D-28 | ACCEPTED (2026-08-30) | P8 recorded permissionless deployment as the eventual destination, which assumed the capability question was settled. Reopened explicitly rather than left to contradict the method | Direction Decision 0 reaching an outcome, at which point P8 is re-decided or re-affirmed |

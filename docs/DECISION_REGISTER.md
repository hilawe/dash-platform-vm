...
| D-27 | Direction Decision 0 is a direction decision, not a gate, and its method is committed before its corpus | **PROPOSED, AWAITING OWNER APPROVAL** | Criteria written after evidence describe the evidence. The method is `docs/DIRECTION_DECISION_0_METHOD.md` and contains no conclusion | None. This one exists to keep the ordering honest |
| D-28 | The direction question has two axes, capability and admission, with admission split into add, invoke and change | **PROPOSED, AWAITING OWNER APPROVAL** | A single four-option list conflated them, which cannot express a restricted general VM or a permissionless native catalog | None foreseen |
| D-29 | P8 is REOPENED by D-28 | ACCEPTED (2026-08-30). Stays accepted regardless of the method's fate, since the contradiction P8 carried is real either way | P8 recorded permissionless deployment as the eventual destination, which assumed the capability question was settled. Reopened explicitly rather than left to contradict the method | Direction Decision 0 reaching an outcome, at which point P8 is re-decided or re-affirmed |
...
| D-10 | Target a supported CosmWasm line for RESEARCH, currently 3.0.x | PROPOSED | SUPERSEDED IN PART by D-16 and D-20, which set the runway policy and concluded 3.0.x is a research target rather than an integration one. What remains proposed is only whether to port research onto it now or wait for 3.1.x to stabilize | 3.1.x stabilizing with adequate runway |

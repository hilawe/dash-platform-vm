...
| P10 | Under whose identity does a program write native collections, and what confines it to document types it should touch? | **OPEN.** Raised by the B7 feasibility check 2026-08-30, which found the Drive layer takes owner identity as DATA while enforcement lives above it in validation driven by the signed state transition. So a host operation that called Drive directly would give programs more authority than users have. The options are a program's own identity, a user's delegated identity, or a restricted set of program-owned contracts. Engine-agnostic, so it must be answered whichever engine wins, and it should be answered before the host operation is built rather than discovered during it. A second part, whether the operation runs the data triggers or explicitly restricts programs to contracts without them, is recorded as D-26 |
| P8 | Deployment permission model | **REOPENED 2026-08-30 by Direction Decision 0.** It read DECIDED, governance-gated at launch and permissionless later. That decision assumed the CAPABILITY question was already settled in favour of a general VM, leaving admission a rollout detail. Direction Decision 0 separates capability from admission and treats admission as genuinely open, so permissionless deployment is now one option of three rather than the destination. Reopened explicitly rather than left to contradict the method document |

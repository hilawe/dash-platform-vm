...
| Proven execution, verify rather than re-execute | Architectural control arm, added 2026-08-30 | Not an engine choice. Validators verify a proof of off-chain execution instead of re-executing, which DISSOLVES the determinism gate rather than passing it, and breaks the assumption that execution fits the block path. Carries a new prover role and an unresolved fee model. Recorded in `docs/GATE3_ZK_SCREEN.md` and not evaluated |
